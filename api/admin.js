...
function requireMinTier(userRow, minTierName) {
  ...
  const userTier = tierLevels[userRow?.plan?.toLowerCase()] || 0
  const requiredTier = tierLevels[minTierName.toLowerCase()] || 0
  
  if (userTier < requiredTier) {
    throw new ApiError(`Fitur ini minimal untuk plan ${minTierName}`, 403)
  }
  
  // Cek expired untuk paid plans (starter, pro, business)
  if (userTier > 0) {
...
function getFotoLimit(plan) {
  ...
  if (userPlan === 'pro' || userPlan === 'business') return -1 // unlimited
  if (userPlan === 'starter') return 5
  return 2 // free
}

function validateFotoLimit(fotoData, maxFoto) {
  if (maxFoto === -1) return // unlimited, skip validation
...
// ================================================
// ORDER ID GENERATOR (NEW - TASK 1.1)
// ================================================
...
async function generateOrderId() {
  ...
  // Get today's sequence dengan retry logic untuk handle race condition
  let sequence = 1
  let attempts = 0
  const maxAttempts = 3
  ...
  while (attempts < maxAttempts) {
    ...
    // Format: EXR-20260709-001
    const orderId = `EXR-${dateStr}-${String(sequence).padStart(3, '0')}`
    
    // Check if order_id already exists (race condition check)
    ...
    // Kalau udah ada, increment dan retry
    attempts++
  }
  
  // Fallback: tambah timestamp untuk uniqueness
...
function mapUser(u) {
  ...
  return {
    ...
    termsAcceptedAt: u.terms_accepted_at || null,  // ✅ TAMBAHKAN INI
    termsVersion: u.terms_version || null,          // ✅ TAMBAHKAN INI
...
const authApi = {
  loginWithGoogle: async (googleUser) => {
    // Cek apakah user udah ada (untuk handle user lama vs baru)
    ...
    // ✅ TAMBAHKAN: Simpan timestamp persetujuan HANYA kalau user baru ATAU user lama yang setuju
    if (googleUser.agreedToTerms) {
      userData.terms_accepted_at = new Date().toISOString()
      userData.terms_version = 'v1.0'
    } else if (!existingUser) {
      // User baru WAJIB setuju (ini backup validation, frontend udah validasi)
      throw new ApiError('Anda harus menyetujui Syarat Layanan dan Kebijakan Privasi', 400)
    }
    // Kalau user lama yang belum pernah setuju, biarin aja (gak overwrite)
...
const tokoApi = {
 ...
 update: async (token, tokoId, data) => {
    ...
    const { error } = await supabaseAdmin
      .from('toko')
      .update({ ...rest, custom_domain: normalizedDomain, updated_at: new Date().toISOString() })
...
const produkApi = {
  ...
  update: async (token, produkId, data) => {
    ...
    // VALIDASI FOTO LIMIT (SATPAM) - hanya kalau field foto di-update
...
const pesananApi = {
  create: async (data) => {
    // GENERATE ORDER ID (NEW - TASK 1.1)
    const orderId = await generateOrderId()
    
    const { data: pesanan, error } = await supabasePublic
      .from('pesanan')
      .insert({
        order_id: orderId, // NEW FIELD
  ...
  // NEW ENDPOINT - TASK 1.1: Get by Order ID (Public Access)
...
// ================================================
// PAYMENT API (DANA MOCK / SIMULATION MODE)
// ================================================

const paymentApi = {
  // 1. Buat Transaksi Upgrade Plan
  createUpgradePayment: async (token, { targetPlan, durationMonths = 1 }) => {
    const userId = await verifyToken(token)
    ...
    const currentPlan = (userRow?.plan || 'free').toLowerCase()
    if (currentPlan === targetPlan.toLowerCase()) {
      throw new ApiError(`Kamu sudah berada di plan ${targetPlan}`, 400)
    }

    // Hitung harga berdasarkan CONFIG (kita hardcode dulu sesuai config lo)
    const prices = { starter: 29000, pro: 49000, business: 79000 }
    const amount = (prices[targetPlan.toLowerCase()] || 0) * durationMonths
    
    if (amount <= 0) throw new ApiError('Plan atau harga tidak valid', 400)

    // Generate Mock Reference ID
    const mockRefId = `DANA-MOCK-${Date.now()}`
    const expiresAt = new Date()
    expiresAt.setMinutes(expiresAt.getMinutes() + 30) // Expired 30 menit
    ...
    const { data: payment, error } = await supabaseAdmin
      ...
      .insert({
        ...
        type: 'upgrade_plan',
        amount: amount,
        target_plan: targetPlan.toLowerCase(),
        duration_months: durationMonths,
        status: 'pending',
        payment_method: 'dana',
        gateway_reference_id: mockRefId,
        expires_at: expiresAt.toISOString(),
        gateway_response: { mock: true, message: 'Simulasi DANA Checkout' }
      })
      .select()
      .single()
    ...
    return { 
      ...
      data: {
        ...
        targetPlan: payment.target_plan,
        danaCheckoutUrl: `https://mock.dana.id/checkout/${mockRefId}`, // URL PALSU buat testing
        danaQrString: '00020101021126580016ID.CO.DANA.WWW...', // QR PALSU
        expiresAt: payment.expires_at
      }
    }
  },

  // 2. Cek Status Pembayaran (Frontend akan polling ini)
  checkPaymentStatus: async (token, paymentId) => {
    ...
    if (error || !payment) throw new ApiError('Transaksi tidak ditemukan', 404)

    // === LOGIKA MOCK: OTOMATIS SUKSES SETELAH 10 DETIK ===
    // (Hanya untuk testing, nanti diganti jadi cek ke DANA API beneran)
    if (payment.status === 'pending') {
      const createdTime = new Date(payment.created_at).getTime()
      const now = Date.now()
      
      // Kalau udah lewat 10 detik sejak dibuat, anggap sukses (Simulasi User Bayar)
      if (now - createdTime > 10000) { 
        await paymentApi._activatePlan(userId, payment)
        return { success: true, data: { ...payment, status: 'paid', mockSimulated: true } }
      }
    }
    // ==================================================
  ...
  // --- HELPER INTERNAL: AKTIFKAN PLAN ---
  _activatePlan: async (userId, payment) => {
    if (payment.status === 'paid') return // Mencegah double activate

    const expiry = new Date()
    // Tambah durasi bulan ke tanggal sekarang
    expiry.setMonth(expiry.getMonth() + payment.duration_months)
    ...
    // Sinkronisasi ke tabel toko (kalau ada)
    const { data: toko } = await supabaseAdmin.from('toko').select('id').eq('user_id', userId).single()
    if (toko) {
      await supabaseAdmin
        .from('toko')
        .update({ plan: payment.target_plan, updated_at: new Date().toISOString() })
        .eq('id', toko.id)
    }

    // Update status payment jadi 'paid'
    await supabaseAdmin
      .from('payments')
...
const REGISTRY = {
  ...
  trafficApi, bundleApi, flashSaleApi, voucherApi, paymentApi
}
