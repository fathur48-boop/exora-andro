...
import { CONFIG, TIER_ORDER, getTierLevel } from '../config.js'
...
function requireMinTier(userRow, minTierName) {
  const userTierLevel = getTierLevel(userRow?.plan)
  const requiredTierLevel = getTierLevel(minTierName)
...
// ================================================
// FOTO LIMIT HELPERS (SATPAM FOTO)
// ================================================
...
export const tokoApi = {
  create: async (token, data) => {
    ...
    const { data: existingTokos } = await supabaseAdmin
      .from('toko')
      .select('id')
      .eq('user_id', userId)
    
    const maxToko = userRow?.plan === 'business' ? 5 : 1
    if ((existingTokos || []).length >= maxToko) {
      throw new ApiError(`Limit toko untuk plan ${userRow?.plan || 'free'} adalah ${maxToko}. Upgrade untuk tambah toko.`, 403)
    }
...
export const produkApi = {
  create: async (token, data) => {
    ...
    const { data: existingProduk } = await supabaseAdmin
      .from('produk')
      .select('id')
      .eq('user_id', userId)
    
    const maxProduk = userRow?.plan === 'pro' || userRow?.plan === 'business' 
      ? -1 
      : userRow?.plan === 'starter' ? 100 : 25
    
    if (maxProduk !== -1 && (existingProduk || []).length >= maxProduk) {
      throw new ApiError(`Limit produk untuk plan ${userRow?.plan || 'free'} adalah ${maxProduk}. Upgrade untuk tambah lebih banyak.`, 403)
    }

    // VALIDASI FOTO LIMIT (SATPAM FOTO)
  ...
  update: async (token, produkId, data) => {
    ...
    // VALIDASI FOTO LIMIT (SATPAM FOTO) - hanya kalau field foto di-update
...
export const adminApi = {
  ...
  grantPlan: async (token, targetUserId, planName, months) => {
    await verifyToken(token)
    
    const validPlans = ['starter', 'pro', 'business']
    if (!validPlans.includes(planName?.toLowerCase())) {
      throw new ApiError('Plan tidak valid', 400)
    }
    ...
    const { error: userErr } = await supabaseAdmin
      .from('users')
      .update({ plan: planName.toLowerCase(), plan_expiry: expiry.toISOString(), updated_at: new Date().toISOString() })
      .eq('id', targetUserId)
    ...
    const { error: tokoErr } = await supabaseAdmin
      .from('toko')
      .update({ plan: planName.toLowerCase(), updated_at: new Date().toISOString() })
      .eq('id', tokoData.id)

    if (tokoErr) handleError(tokoErr)

    return { success: true, message: `${planName} aktif ${months} bulan` }
  },

  grantPro: async (token, targetUserId, months) => {
    return adminApi.grantPlan(token, targetUserId, 'pro', months)
  },
...
export const showcaseChatApi = {
  send: async ({ messages, posts, produkList }) => {
    ...
    const produkStr = (produkList || []).slice(0, 50).map((p, i) =>
      `${i + 1}. ${p.nama} — Rp ${Number(p.harga).toLocaleString('id-ID')}${p.stok === 0 ? ' (Habis)' : ''}${p.kategori ? ' [' + p.kategori + ']' : ''} — Toko: ${p.tokoNama || ''}${p.tokoSlug ? ' (exorashop.vercel.app/toko/' + p.tokoSlug + ')' : ''}`
    ).join('\n')
...
Jawab ramah, singkat, dalam Bahasa Indonesia. Jika buyer mau beli atau tanya toko, selalu sertakan link lengkap format: https://exorashop.vercel.app/toko/[slug]`
