...
import { Heart, MessageCircle, Repeat2, Store, ArrowRight, Bot, Sun, Moon, X, ZoomIn, Loader2, ChevronDown, ChevronUp, Crown, Sparkles } from 'lucide-react'
import { useStreamStore } from '../lib/store.js'
import { getStorefrontUrl, getInitials, getTierLevel, getPlanDisplayName } from '../lib/utils.js'
...
const THEMES = {
  light: {
    ...
    proText: '#92400e',
    starterBg: 'rgba(59,130,246,0.15)',
    starterBorder: 'rgba(59,130,246,0.35)',
    starterText: '#1e40af',
    businessBg: 'rgba(16,185,129,0.15)',
    businessBorder: 'rgba(16,185,129,0.35)',
    businessText: '#065f46',
  ...
  dark: {
    ...
    proText: '#fbbf24',
    starterBg: 'rgba(59,130,246,0.18)',
    starterBorder: 'rgba(59,130,246,0.4)',
    starterText: '#60a5fa',
    businessBg: 'rgba(16,185,129,0.18)',
    businessBorder: 'rgba(16,185,129,0.4)',
    businessText: '#34d399',
...
function ShowcaseCard({ post, theme, c, index, onAuthRequired, onImageClick, onTagClick, activeTag, isExpanded, onToggleExpand }) {
  ...
  const tierLevel = t?.plan ? getTierLevel(t.plan) : (t?.pro ? 2 : 0)
  const showBadge = tierLevel >= 1
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
            ...
            {showBadge && <TierBadge tierLevel={tierLevel} theme={theme} c={c} />}
          </div>
...
function TierBadge({ tierLevel, theme, c }) {
  ...
  if (tierLevel >= 3) {
    ...
    label = 'Business'
    bg = c.businessBg
    border = c.businessBorder
    textColor = c.businessText
  } else if (tierLevel >= 2) {
    icon = '⭐'
    label = 'Pro'
    bg = c.proBg
    ...
    label = 'Starter'
    bg = c.starterBg
    border = c.starterBorder
    textColor = c.starterText
  } else {
    return null
  }
