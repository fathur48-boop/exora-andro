...
import { formatRupiah, getTierLevel, getPlanDisplayName } from '../lib/utils.js'
...
export default function AnalyticsPage() {
  ...
  // 4-tier logic: minimal STARTER (tierLevel >= 1)
  const tierLevel = getTierLevel(user?.plan)
  const planName = getPlanDisplayName(user?.plan)
  
  if (tierLevel < 1) return <AnalyticsGate currentPlan={planName} />
  return <AnalyticsDashboard tokenObj={token} />
}

function AnalyticsGate({ currentPlan }) {
  return (
      ...
      >
        <div style={{
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.3rem', marginBottom: 10 }}>
            Analytics — Fitur {currentPlan === 'Free' ? 'Starter' : 'Premium'}
          </h2>
