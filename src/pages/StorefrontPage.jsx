...
import { formatRupiah, generateCheckoutMessage, generateWALink, validateWA, truncate, generateShareProdukWA, getTierLevel } from '../lib/utils.js'
...
// ==========================================
// NEW: WhatsApp Order Message Generator (FITUR 3)
// ==========================================

/**
 * Generate pesan WA yang rapi dan profesional untuk checkout
 * Format baru dengan order_id, struktur jelas, dan link invoice
 */
function generateOrderMessage(produk, toko, buyer, orderId, voucherApplied = null) {
  ...
  if (buyer.catatan) {
    lines.push(`Catatan: ${buyer.catatan}`)
  }
  
  lines.push('')
  lines.push('📦 *DETAIL PRODUK*')
  lines.push(`• ${produk.nama}`)
  lines.push(`  Qty: ${buyer.qty || 1} × ${formatRupiah(produk.harga)}`)
  lines.push(`  Subtotal: ${formatRupiah(subtotal)}`)
  
  if (voucherApplied) {
    lines.push('')
    lines.push('🎟️ *VOUCHER*')
    lines.push(`Kode: ${voucherApplied.kode}`)
    lines.push(`Diskon: -${formatRupiah(diskon)}`)
  }
  
  lines.push('')
  lines.push('━━━━━━━━━━━━━━━━━━')
  lines.push(`💰 *TOTAL BAYAR: ${formatRupiah(total)}*`)
  lines.push('━━━━━━━━━━━━━━━━━━')
  lines.push('')
  lines.push('🔗 *Invoice:*')
  lines.push(invoiceUrl)
  lines.push('')
  lines.push('Mohon konfirmasi ketersediaan ya! 🙏')
...
// ========== TAMBAHAN: TIER BADGE COMPONENT (4-TIER SYSTEM) ==========
function TierBadge({ plan, tema, c, theme }) {
  const tierLevel = plan ? getTierLevel(plan) : 0
  if (tierLevel === 0) return null
  
  let icon, label, bg, border, textColor
  ...
  if (tierLevel >= 3) {
    ...
    textColor = theme === 'light' ? '#065f46' : '#34d399'
  } else if (tierLevel >= 2) {
    icon = '⭐'
    label = 'Pro'
    bg = 'rgba(251,191,36,0.15)'
    border = 'rgba(251,191,36,0.35)'
    textColor = theme === 'light' ? '#92400e' : '#fbbf24'
  } else if (tierLevel >= 1) {
    icon = '✨'
    label = 'Starter'
    bg = 'rgba(59,130,246,0.15)'
    border = 'rgba(59,130,246,0.35)'
    textColor = theme === 'light' ? '#1e40af' : '#60a5fa'
  } else {
    return null
  }
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 2 }}>
                ...
                <TierBadge plan={toko.plan} tema={tema} c={c} theme={theme} />
              </div>
...
// ==========================================
// UPDATED: CheckoutModal dengan Order ID (FITUR 3)
// ==========================================
function CheckoutModal({ produk: p, toko, tema, accentColor, c, onClose, getFlashInfo }) {
  ...
  const [submitting, setSubmitting] = useState(false)
  const [orderSuccess, setOrderSuccess] = useState(null) // NEW: State untuk success dengan order_id
  ...
  const handleCheckout = async () => {
    ...
    setSubmitting(true)
    try {
      // 1. Simpan pesanan ke backend (backend auto-generate order_id)
      ...
      // 2. Ambil order_id dari response
      const orderId = res.data?.orderId || res.data?.order_id || res.data?.id
      
      // 3. Generate pesan WA dengan format baru (pakai generateOrderMessage)
      const message = generateOrderMessage(p, toko, form, orderId, voucherApplied)
      
      // 4. Set success state untuk tampilkan order_id
      setOrderSuccess({
        orderId,
        produk: p.nama,
        total: hargaEfektif * form.qty,
      ...
      // 5. Buka WhatsApp dengan pesan terstruktur
      const waLink = generateWALink(toko.wa, message)
      window.open(waLink, '_blank')
      
      // 6. Redeem voucher kalau dipakai
      if (voucherApplied?.voucherId) {
        voucherApi.redeem(voucherApplied.voucherId).catch(() => {})
      }
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            <p style={{ fontSize: '0.72rem', color: c.textTertiary, margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>
              Order ID
            </p>
            <p style={{ 
          ...
          <div style={{
            ...
            {orderSuccess.voucherApplied && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ fontSize: '0.82rem', color: c.textTertiary }}>Voucher</span>
                <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#10b981' }}>
                  -{formatRupiah(orderSuccess.voucherApplied.diskon)}
                </span>
              </div>
            )}
            <div style={{ 
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <MagneticButton strength={0.2} style={{ width: '100%' }}>
            ...
            >
              ...
              {submitting ? 'Menyimpan...' : 'Pesan via WhatsApp'}
            </motion.button>
          </MagneticButton>
