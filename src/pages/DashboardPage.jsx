...
import { formatRupiah, getStorefrontUrl, isPro, getTierLevel, getPlanDisplayName, getProductLimit, copyToClipboard, generateShareTokoWA } from '../lib/utils.js'
...
export default function DashboardPage() {
  ...
  const isPaidUser = tierLevel > 0 // Starter, Pro, atau Business
  const maxProduk = getProductLimit(user?.plan) // 25, 100, atau -1 (unlimited)
  const planName = getPlanDisplayName(user?.plan)

  const sisaHari = isPaidUser && user?.planExpiry
      ...
      {tokoLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <AnimatePresence mode="popLayout">
            {/* Banner Upgrade - Hanya untuk FREE user */}
            {tierLevel === 0 && (
              <motion.div
                key="upgrade-banner"
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>
                    Mulai dari Rp 39.000/bulan. Fitur lebih banyak, produk lebih banyak, dan akses ke analytics.
                  </p>
                </div>
                <Link
            ...
            {/* Banner Expiry - Untuk semua paid user */}
            {isPaidUser && sisaHari !== null && sisaHari <= 7 && (
              <motion.div
                key="expiry-banner"
                initial={{ opacity: 0, y: -10, height: 0 }}
                animate={{ opacity: 1, y: 0, height: 'auto' }}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  <p style={{
                    ...
                    {sisaHari <= 0 ? `Paket ${planName} kamu sudah expired` : `Paket ${planName} berakhir dalam ${sisaHari} hari`}
                  </p>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>
                    {sisaHari <= 0
                      ? 'Perpanjang sekarang agar toko dan fitur tetap aktif.'
                      : 'Segera perpanjang agar tidak kehilangan akses ke fitur premium.'}
                  </p>
                </div>
                <a
                  href={'https://wa.me/' + CONFIG.ADMIN_WA + '?text=' + encodeURIComponent(`Halo, saya mau perpanjang paket ${planName} Exora.`)}
                  target="_blank"
                  rel="noreferrer"
            ...
            {/* Banner Limit - Dinamis sesuai tier */}
            {limitReached && (
              <motion.div
                key="limit-banner"
                initial={{ opacity: 0, y: -10, height: 0 }}
          ...
          >
            ...
            <motion.div variants={statItemVariants}>
              <StatCard
                ...
                trend={tierLevel >= 1 ? '' : 'Upgrade untuk akses'}
                color="var(--accent-3)"
              />
            </motion.div>
          </motion.div>
