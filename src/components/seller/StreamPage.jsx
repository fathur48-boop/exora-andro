...
import { getStorefrontUrl, getInitials, getTierLevel } from '../../lib/utils.js'
import { tokoApi } from '../../lib/api/adminClient.js'
...
export default function StreamPage() {
  ...
  // UPDATE TIER LOGIC: Minimal STARTER (tierLevel >= 1)
  ...
  const requireAccess = () => toast.error('Fitur ini minimal untuk paket Starter')
  ...
  const openDm = async (otherTokoId) => {
    if (!hasAccess) return requireAccess()
    try {
      ...
      <div style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        {!hasAccess && (
          <div style={{
            ...
            Kamu bisa melihat Stream, tapi posting/balas/like/DM minimal paket Starter.
          </div>
        )}
        ...
        <motion.button
          ...
          style={{
            ...
            background: hasAccess ? 'var(--accent-gradient)' : 'var(--surface)',
            border: hasAccess ? 'none' : '2px solid var(--glass-border)',
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: hasAccess ? '0 4px 24px var(--accent-glow, rgba(91,138,245,0.4))' : 'none',
            zIndex: 30,
          }}
        >
          {hasAccess ? <span style={{ color: '#fff', fontSize: 24, lineHeight: 1, marginTop: -2 }}>+</span> : <Lock size={18} color="var(--text-tertiary)" />}
        </motion.button>
        ...
        {composing && (
          <ComposeSheet
            tokenObj={tokenObj}
            feed={feed}
            plan={user?.plan || 'free'}  // ✅ FIX 1: Kirim prop plan ke ComposeSheet
...
// POST CARD
// ================================================
function PostCard({ post, index, myTokoId, hasAccess, onExpand, onLike, onRepost, onBookmark, onReply, onReplyToComment, onDm, onTag, onDelete, isExpanded, onToggleExpand }) {
  const t = post.toko
...
// POST DETAIL VIEW
// ================================================
function PostDetailView({ post, loading, myTokoId, hasAccess, onBack, onLike, onRepost, onBookmark, onReply, onDm, onTag, onDelete, isExpanded, onToggleExpand }) {
...
// ================================================
function ComposeSheet({ tokenObj, feed, plan, onClose, onSubmit }) {  // ✅ FIX 2: Tambah param plan
  ...
  return (
    <Sheet onClose={onClose} title="Buat Post">
      ...
      <StreamImageUpload 
        ...
        tokenObj={tokenObj} 
        disabled={submitting}
        plan={plan}  // ✅ FIX 3: Kirim prop plan ke StreamImageUpload
      />
...
// PostText dengan fitur See More & Mention Rendering
function PostText({ text, onTag, isExpanded, onToggleExpand }) {
  ...
  const renderContent = (content) => {
    ...
    // Split by mention pattern @slug (alphanumeric + hyphen)
    ...
    <div>
      <p style={{ fontFamily: PJS, fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.7, margin: '0 0 5px', whiteSpace: 'pre-line' }}>
        {renderContent(displayText).map((node, i) => {
           // Handle hashtags within the text nodes if they aren't split by the mention regex
           // For simplicity, we rely on the split. If a part contains #, we can split further or just render.
           // Let's do a simple split for hashtags on non-mention parts.
